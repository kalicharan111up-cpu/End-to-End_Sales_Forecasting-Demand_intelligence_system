...
# ==================== PAGE 2 ====================
    ...
    st.caption(f"Best model in benchmark: **{res['best_model']}** · "
               f"Segment forecasts below use Prophet (robust across all segments).")

    scope = st.selectbox("Scope",
        ["Overall", "Category · Furniture", "Category · Technology",
         "Category · Office Supplies", "Region · West", "Region · East"])
    horizon = st.slider("Forecast horizon (months)", 1, 3, 3)

    if scope == "Overall":
        # SARIMA (best model) top-level forecast
        hist_dates = monthly_df["date"].tolist()
        hist_vals = monthly_df["Sales"].tolist()
        fc = res["models"]["SARIMA"]
        fc_dates = res["models"]["forecast_dates"][:horizon]
        fc_vals = fc["forecast"][:horizon]
        lower = fc["lower"][:horizon]
        upper = fc["upper"][:horizon]

        fig = go.Figure()
        fig.add_trace(go.Scatter(x=hist_dates, y=hist_vals, name="Actual",
                                 line=dict(color="steelblue")))
        fig.add_trace(go.Scatter(x=fc_dates, y=fc_vals, name="SARIMA forecast",
                                 line=dict(color="crimson", dash="dash"), mode="lines+markers"))
        fig.add_trace(go.Scatter(x=fc_dates + fc_dates[::-1],
                                 y=upper + lower[::-1],
                                 fill="toself", fillcolor="rgba(220,20,60,0.15)",
                                 line=dict(color="rgba(255,255,255,0)"),
                                 hoverinfo="skip", showlegend=True, name="95% CI"))
        fig.update_layout(title=f"Overall Monthly Sales Forecast ({horizon} mo ahead)",
                          xaxis_title="Date", yaxis_title="Sales ($)")
        st.plotly_chart(fig, use_container_width=True)

        c1, c2, c3 = st.columns(3)
        c1.metric("Test MAE",  f"${fc['MAE']:,.0f}")
        c2.metric("Test RMSE", f"${fc['RMSE']:,.0f}")
        c3.metric("Test MAPE", f"{fc['MAPE']:.1f}%")

        st.subheader("Forecast values")
        st.dataframe(pd.DataFrame({
            "Date": fc_dates, "Forecast ($)": [f"{v:,.0f}" for v in fc_vals],
            "Lower 95%": [f"{v:,.0f}" for v in lower],
            "Upper 95%": [f"{v:,.0f}" for v in upper]}), hide_index=True)
    else:
        name = scope.split("·")[1].strip()
        # Map to the segment_forecasts keys
        key_map = {"Furniture": "Furniture", "Technology": "Technology",
                   "Office Supplies": "Office Supplies", "West": "West Region",
                   "East": "East Region"}
        key = key_map[name]
        s = res["segment_forecasts"][key]
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=s["history_dates"], y=s["history"], name="Actual",
                                 line=dict(color="steelblue")))
        fig.add_trace(go.Scatter(x=s["forecast_dates"][:horizon],
                                 y=s["forecast"][:horizon],
                                 name="Prophet forecast",
                                 line=dict(color="crimson", dash="dash"),
                                 mode="lines+markers"))
        fig.update_layout(title=f"{key} — {horizon}-Month Forecast",
                          xaxis_title="Date", yaxis_title="Sales ($)")
        st.plotly_chart(fig, use_container_width=True)
        st.dataframe(pd.DataFrame({
            "Date": s["forecast_dates"][:horizon],
            "Forecast ($)": [f"{v:,.0f}" for v in s["forecast"][:horizon]]}),
            hide_index=True)
        st.info("Segment metrics use the overall SARIMA benchmark as a reference; the segment "
                "model is Prophet re-fit per segment.")
